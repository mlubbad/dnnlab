...
saved_model_dir = "/home/dnnlab/muhammed/%s/saved_model/EFFICIENTNET_b4_best_model.pt" % (model_name)
model_path = "/home/dnnlab/muhammed/%s/saved_model/EFFICIENTNET_b4_best_model.pt" % (model_name)
#model_path = "/home/dnnlab/muhammed/%s/saved_model" % (model_name)
TRAIN = 'test'
VAL = 'test'
TEST = 'test'
batch_size = 16
...
model.classifier.fc = nn.Linear(num_features, len(class_names))
"""
# Freeze training for all layers
for param in model.features.parameters():
    param.require_grad = False
...
print("\rClassification Report:\n", report)
cf_matrix = confusion_matrix(y_preds,y_true)
sns.heatmap(cf_matrix, cmap='coolwarm', annot=True, linewidth=1, fmt="d")
...
plot_heatmap(y_true, y_preds, class_names, ax1, title="Transfer Learning (EFFICIENTNET_b4)")    
