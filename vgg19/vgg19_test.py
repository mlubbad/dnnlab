...
model_path = '/home/dnnlab/muhammed/vgg19/saved_model/VGG19_best_model.pt'
TRAIN = 'test'
VAL = 'test'
TEST = 'test'
batch_size = 16
...
def load_and_test(model_path, I):
    model = models.vgg19()
...
model = torch.hub.load('pytorch/vision:v0.11.0', 'vgg19_bn', pretrained=True)
# Freeze training for all layers
for param in model.features.parameters():
    param.require_grad = False
# Newly created modules have require_grad=True by default
...
plot_heatmap(y_true, y_preds, class_names, ax1, title="Transfer Learning (VGG19) After Dyna Class Augmentation #2 Trial")    
