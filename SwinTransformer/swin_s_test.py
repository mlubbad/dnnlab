...
model_path = '/home/dnnlab/muhammed/SwinTransformer/saved_model/swin_s_best_model.pt'
TRAIN = 'test'
VAL = 'test'
TEST = 'test'
batch_size = 16
...
print('-' * 10)
model = models.swin_s(weights='DEFAULT', progress=True)
...
#model.heads[0] = nn.Linear(1536, len(class_names))
#model.head = nn.Linear(1024, len(class_names))
model.head = nn.Linear(768, len(class_names))
"""
...
plot_heatmap(y_true, y_preds, class_names, ax1, title="Transfer Learning (SwinTransformer_Small)")    
