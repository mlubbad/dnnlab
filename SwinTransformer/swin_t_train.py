...
#model = torch.hub.load('pytorch/vision:v0.13.0', 'swin_t', pretrained=True)
model = models.swin_t(weights='DEFAULT', progress=True)
print(model)
...
if resume_training:
    ...
    model.load_state_dict(torch.load(f'{saved_model_dir}/swin_t_best_model.pt'))
    print("Loaded!")
...
torch.save(model.state_dict(), f'{saved_model_dir}/swin_t_best_model.pt')
